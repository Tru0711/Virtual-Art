...
export class MuseumLayout { 
  ...
  getArtistWallPlacements(artistId, artworks) {
    // Create a unique deterministic “wing” per artist: front left, front right, then rear rooms.
    ...
    // Build placements row/column packing and no overlap.
    // Plan: place on one long wall per artist.
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      // z plane offset depends on wall orientation
      ...
      if (Math.abs(cfg.rotationY) < 1e-6) {
        ...
        // Invert and rotate based on cfg
        position = [x, y, cfg.pos.z - wallLocalZ];
  ...
  _sectionSlots(artworkCount) {
    // Cap to keep it performant.
    const capped = Math.min(artworkCount, 48);
    const cols = 4; // fixed for premium look
    const rows = Math.max(1, Math.ceil(capped / cols));
    return { cols, rows };
  }
}

MuseumLayout.Shell = function Shell({ museum }) {
